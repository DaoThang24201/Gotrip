...
function priceRangeSliderInit() {
  ...
  targets.forEach(el => {
    ...
    slider.noUiSlider.on('update', function (values, handle) {
     ...
     /* slider.noUiSlider.on('update', function (values, handle) {
          const cleanedValues = values.map(value => value.replace('$', '')); // Loại bỏ ký tự "$"

          snapValues[handle].innerHTML = values[handle];

          // Cập nhật giá trị lên URL
          //updateUrl(cleanedValues);
      });*/
      ...
      /*function updateUrl(values) {
          ...
          // Sử dụng URLSearchParams để thao tác với parameter
          const params = new URLSearchParams(url.search);

          // Đặt giá trị parameter
          params.set('min_price', lowerValue);
          params.set('max_price', upperValue);

          // Cập nhật URL với parameter mới
